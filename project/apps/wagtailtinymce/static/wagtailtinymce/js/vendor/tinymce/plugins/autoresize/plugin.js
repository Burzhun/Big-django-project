...
var autoresize = (function () {
  ...
  var resize = function (editor, oldSize) {
    ...
    body = doc.body;
    resizeHeight = $_e2hnnp82jcg9ma5p.getAutoResizeMinHeight(editor);
    ...
    if (myHeight > $_e2hnnp82jcg9ma5p.getAutoResizeMinHeight(editor)) {
      resizeHeight = myHeight;
    }
    var maxHeight = $_e2hnnp82jcg9ma5p.getAutoResizeMaxHeight(editor);
  ...
  var setup = function (editor, oldSize) {
    editor.on('init', function () {
      ...
      var dom = editor.dom;
      overflowPadding = $_e2hnnp82jcg9ma5p.getAutoResizeOverflowPadding(editor);
      bottomMargin = $_e2hnnp82jcg9ma5p.getAutoResizeBottomMargin(editor);
    ...
    if ($_e2hnnp82jcg9ma5p.shouldAutoResizeOnInit(editor)) {
      editor.on('init', function () {
  ...
  var register = function (editor, oldSize) {
    editor.addCommand('mceAutoResize', function () {
      $_1rftsh7zjcg9ma5m.resize(editor, oldSize);
    });
  };
  var $_e2deal7yjcg9ma5j = { register: register };

  PluginManager.add('autoresize', function (editor) {
    if (!editor.inline) {
      var oldSize = Cell(0);
      $_e2deal7yjcg9ma5j.register(editor, oldSize);
      $_1rftsh7zjcg9ma5m.setup(editor, oldSize);
    }
  });
  var Plugin = function () {
  };
