{% load i18n %}
function(modal) {
    var searchUrl = $('form.giphy-search', modal.body).attr('action');
    ...
    function ajaxifyLinks (context) {
        $('.listing a', context).click(function() {
            modal.postForm(this.href, {'json': $(this).data('json')});
            return false;
        });
    ...
    function fetchResults(requestData) {
        $.ajax({
            ...
            success: function(data, status) {
                $('#giphy-results').html(data);
                ajaxifyLinks($('#giphy-results'));
            }
        });
    }
    ...
    $('form.giphy-search', modal.body).submit(search);
